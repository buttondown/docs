...
const dateRangeDescription = (parameters: ParameterType[]) => {
  const lines = parameters.map((parameter) => {
    ...
    const formatLabel = parameter.format === "date-time" ? "datetime" : "date";
    const example =
  ...
  if (page.method && page.endpoint) {
    ...
    return (
      <Layout slug={slug} title={page.title}>
        ...
        />
        <h2 className="mb-1" id="sample-requests">
          Sample requests
        </h2>
        <p>
        ...
        <hr />
        <h2 className="mb-1" id="sample-responses">
          Sample responses
        </h2>
        <p>
        ...
        {operation.parameters.length > 1 && (
          <>
            <hr />
            <h2 className="mb-1" id="path-parameters">
              Path parameters
            </h2>
            <p>
              Consult the{" "}
            ...
            {groupDateRangeParameters(extractParameters(operation)).map(
              (row) =>
                row.kind === "date-range" ? (
                  <DateRangeParameter
        ...
        {operation.requestBody && (
          <>
            <hr />
            <h2 className="mb-1" id="body-parameters">
              Body parameters
            </h2>
        ...
        {(() => {
          ...
          if (responsesWithCodes.length > 0) {
            return (
              <>
                <hr />
                <h2 className="mb-1" id="error-codes">
                  Error codes
                </h2>
                <p>
  ...
  return (
    <Layout slug={slug} title={page.title}>
      ...
      {page.relatedPages.length > 0 && (
        <div>
          <h2 className="text-xl font-bold text-gray-800 mt-8" id="related">
            Related
          </h2>
          <ul className="mt-4">
      ...
      {slug.includes("api-changelog-") && (
        <div>
          ...
          <p>
            ...
            , or <Link href="/api-introduction">read the API reference</Link>.
          </p>
        </div>
      )}
      <script
